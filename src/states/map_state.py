...
class MapState(State):
    ...
    def update(self, dt):
        ...
        if dist < self.encounter_radius:
            ...
            # Pasamos el nivel actual al combate
            self.game.set_state(CombatState(self.game, self.player, self.level))
    ...
    def render(self, screen):
        ...
        # Personaje (Escalado y dentro del sendero)
        if "image" in self.player:
            img = pygame.transform.scale(self.player["image"], (100, 100))
            # Midbottom en y_line + 10 para que parezca que pisa el centro de la línea de 20px
            rect = img.get_rect(midbottom=(self.player_pos[0], self.y_line + 10))
            screen.blit(img, rect)
        
        # UI
        txt = self.font.render(f"Progreso: {self.level}/5 | {self.player['name']} | HP: {self.player.get('hp', '??')}", True, (255, 255, 255))
